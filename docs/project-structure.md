...
在当前第二块增量里，`application` 还承接了一个刻意保持轻量的教学职责：

- 命令侧 use case 发布领域事件
- 事件被同步投影到查询侧读模型
- 查询 use case 只通过 `TaskQueryService` 读取读模型

这里表达的是教学版 CQRS，而不是完整生产方案：当前实现是**同步、单进程、非事务 outbox** 的最小收口。
...
在第二块 DDD 深化后，命令主线会继续延伸出一小段教学型事件流：

```text
聚合完成业务动作
  -> 产生领域事件
  -> 同步事件总线分发
  -> projector 更新 task_read_models
```
...
这样读者更容易看清：查询并不是“顺手复用仓储查一下”，而是显式走读模型。
这同样意味着当前示例接受一个教学上的边界：如果读模型投影未完成，查询结果就可能暂时落后于写模型。
